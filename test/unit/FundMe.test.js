    ...
    : describe("FundMe", async function () {
          ...
          describe("constructor", async function () {
              it("sets the aggregator addresses correclty", async function () {
                  const response = await fundMe.getPriceFeed() // priceFeed : is the public variable in the contract. So by calling it we can fetch the address of the contract.
          ...
          describe("fund", async function () {
              ...
              it("Adds the funder to s_funders array", async function () {
                  await fundMe.fund({ value: sendValue })
                  const funder = await fundMe.getFunder(0) //check the first index -> should be the deployer
